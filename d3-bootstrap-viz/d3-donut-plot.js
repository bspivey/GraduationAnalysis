export function Donut(county){
    ...
    d3.dsv(",", "df_grad_rate_pct_county_merge.csv", function (d) {
        return {
            ...
            'Indian': Math.floor(+d['AMERICAN INDIAN OR ALASKA NATIVE']),
            'Asian': Math.floor(+d['ASIAN OR PACIFIC ISLANDER']),
            'Black': Math.floor(+d['BLACK']),
            'White' : Math.floor(+d['WHITE']),
            'Hispanic' : Math.floor(+d['HISPANIC']),
            'Multiracial' : Math.floor(+d['MULTIRACIAL']),
            'Economically Disadvantged' : +d['ECONOMICALLY DISADVANTAGED'],
            'Economically Advantged' : +d['NOT ECONOMICALLY DISADVANTAGED'],
            'Female' : Math.floor(+d['FEMALE']),
            'Male' : Math.floor(+d['MALE']),
