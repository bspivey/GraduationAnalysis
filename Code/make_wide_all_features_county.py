...
"""Script to merge all features for statistics and machine learning
    ...
    Creates all features by county
...
cols_to_merge = ['COUNTY_CODE', 'COUNTY_NAME', 'YEAR', 'NUM_OUT_CERT', 'PER_TEACH_INEXP', 'PER_OUT_CERT', 'NRC_CODE']
df_combined_features = df_combined_1[cols_to_merge]
df_combined_features.drop_duplicates(inplace=True)
cols_to_merge_key = ['COUNTY_CODE', 'COUNTY_NAME', 'YEAR']
...
filepath = '../Data/df_grad_rate_pct_county.csv'
df_grad_rate_pct_county = pd.read_csv(filepath)

# Select only one school
#df_grad_rate_pct_school = df_grad_rate_pct_school[df_grad_rate_pct_school['AGGREGATION_NAME'] == 'CENTRAL VALLEY ACADEMY']

df_grad_rate_pct_county = df_grad_rate_pct_county.sort_values(by=cols_to_merge_key) \
                                                 .reset_index() \
                                                 .drop(columns=['index'])

# Merge staff qualification and charter school colums into the wide table
df_grad_rate_pct_county_merge = df_grad_rate_pct_county.merge(df_combined_features, \
